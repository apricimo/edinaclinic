...
export async function handler(event) {
  ...
  const { path, segments } = normalizePath(rawPath, ["ping", "checkout", "services", "providers"]);
...
function normalizePath(rawPath = "/", knownRoots = []) {
  ...
  const segments = pieces.map((piece) => {
    ...
    return decoded === "/" ? decoded.toLowerCase() : decoded;
  });
...
async function createService(payload) {
  ...
  if (!stateCodes.length) fields.state_codes = fields.state_codes || "At least one state must be selected";
  ...
  const now = isoNow();
  const serviceId = generatedId("svc");
  const pk = `SERVICES#${serviceId}`;
...
async function createProvider(payload) {
  ...
  if (unknown.length) {
    return { ok: false, status: 400, body: { ok: false, error: "Validation failed", fields: { offered_service_ids: "Unknown service ids" } } };
  }

  const now = isoNow();
  const providerId = generatedId("prv");
  const item = {
    pk: `PROVIDERS#${providerId}`,
    sk: "v0",
...
async function deleteProvider(providerId) {
  ...
  const hasActive = appointments.some((item) => {
    if (item?.provider_id !== cleanId) return false;
  ...
  await table.updateItems(async (items) => {
    const nextItems = items.filter((item) => {
      ...
      if (item.pk.startsWith(`AVAILS#${cleanId}#`)) return false;
      return true;
    });
    return { items: nextItems, result: null };
  });
...
async function createAvailability(payload) {
  ...
  const pk = `AVAILS#${providerId}#${startDate.toISOString()}`;
  const sk = `SERVICES#${serviceId}`;
  const now = isoNow();
  const item = {
    pk,
    sk,
...
async function deleteAvailability(providerId, startIso, serviceId) {
  ...
  const segments = [cleanProvider, startTime, cleanService];
  const normalised = segments.length ? `/${segments.join("/")}` : "/";

  const pk = `AVAILS#${cleanProvider}#${startTime}`;
  const sk = `SERVICES#${cleanService}`;
  const existing = await table.getItem(pk, sk);
  if (!existing) {
...
async function createAppointment(payload) {
  ...
  const availabilityPk = `AVAILS#${providerId}#${startDate.toISOString()}`;
  const availabilitySk = `SERVICES#${serviceId}`;
  ...
  const appointmentId = generatedId("apt");
    ...
    await table.updateItems(async (items) => {
      const nextItems = items.map((item) => {
        if (item.pk === availabilityPk && item.sk === availabilitySk) {
          ...
          return { ...item, booked_appointment_ids: bookedIds, updated_at: isoNow() };
        }
        return item;
      });
...
async function updateAppointment(appointmentId, payload) {
  ...
  await table.updateItems(async (items) => {
    const nextItems = items.map((item) => (item.pk === pk && item.sk === "v0" ? { ...item, ...updates } : item));
    return { items: nextItems, result: null };
  });

  const services = await loadServiceMap();
  const providers = await loadProviderMap();
  return { ok: true, data: formatAppointment({ ...existing, ...updates }, services, providers) };
}
...
function requiredString(value, field, fields) {
  if (value === undefined || value === null) {
    fields[field] = `${String(field).replace(/_/g, " ")} is required`;
    return undefined;
  }
...
function toInteger(value, field, fields) {
  ...
  if (!Number.isFinite(number)) {
    if (fields && field) fields[field] = `${String(field).replace(/_/g, " ")} must be a number`;
    return undefined;
  }
...
function toStateArray(value, field, fields) {
  ...
  if (field && fields && (value === undefined || !arr.length)) {
    fields[field] = `${String(field).replace(/_/g, " ")} must include at least one state`;
  }
  return arr;
}
